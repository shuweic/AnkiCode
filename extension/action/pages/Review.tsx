            ...
            <Flex vertical gap={16}>
                {
                    data!.data.items.map((item, index) => {
                        ...
                        const leetcodeName = name.trim().toLowerCase().split(' ').join('-');
                        return (
                            <Card size="small" className="problem">
                                <Flex align="center" gap={8}>
                                    <div className="number">#{index + 1}</div>
                                    <Flex vertical justify="center" className="title">
                                        <Link href={`https://leetcode.com/problems/${leetcodeName}/description/`} target="_blank">
